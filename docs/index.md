...
# run app
java -jar target/erick-test-one-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/erick-test-one:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/erick-test-one -n dev

# get external endpoint
kubectl get svc erick-test-one-svc -n dev
```
...
```bash
kubectl port-forward svc/erick-test-one-svc 8080:80 -n dev
...
kubectl rollout restart deploy/erick-test-one -n dev
kubectl rollout status deploy/erick-test-one -n dev
```
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/ericktestone/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
...
© erick.vaquero@morningstar.com. For demo/PoV purposes.
